        ...
        void Draw()
        {
            if (OsmReader.Instance.nodes != null)
            {
                ...
                for (int i = 0; i < intersections.Count; i++)
                {
                    ...
                    var fmh_41_62_638595006956616078 = Quaternion.identity; Handles.FreeMoveHandle(intersections[i], 1f, Vector2.zero, Handles.CylinderHandleCap);
                ...
                foreach (var u in intersection.edgesIntersection)
                {
                    for (int i = 0; i < u.Length; i++)
                    {
                        ...
                        var fmh_53_54_638595007159478407 = Quaternion.identity; Handles.FreeMoveHandle(u[i], 0.5f, Vector2.zero, Handles.CylinderHandleCap);
